...
int totalMalePassengers = 0;
int totalFemalePassengers = 0;
int rejectedMalePassengers = 0;
int rejectedFemalePassengers = 0;
vector<int> maleWaitingTimes(16, 0);
vector<int> femaleWaitingTimes(16, 0);
...
void getTimeSourceDestination() {
	...
	while( getline(file, s) ) 
	{
		...
		ss >> gender;  // Read gender from input
		if (!isValidGender(gender)) {
			gender = (rand() % 2) ? 'M' : 'F';  // Randomly assign if not valid
		}
		...
		if (source < nodeID.size() && dest < nodeID.size() && timeSlot >= startTime / DELTA_TIME) {
			...
			req.gender = gender;  // Set the gender field
	...
	for (unordered_map<int, vector<passenger> >::iterator iter = passengerQueue.begin(); iter != passengerQueue.end(); iter++) {
		...
		for (int i = 0; i < passList.size(); i++) {
			...
			if( (timeSlot - passList[ i ].startTimeSlot)*DELTA_TIME > MINUTE_BEFORE_REJECTION ) {
				...
				globalRejections++;
				if (passList[i].gender == 'M') {
					rejectedMalePassengers++;
				} else {
					rejectedFemalePassengers++;
				}
				continue;
			}
...
void calculateMetrics(int timeSlot) {
	for (int i = 0; i < passengerPickedList.size(); i++) {
		if (passengerPickedList[ i ].pickedTimeSlot == timeSlot) {
			...
			if (passengerPickedList[i].gender == 'M') {
				maleWaitingTimes[waitedSlot]++;
			} else {
				femaleWaitingTimes[waitedSlot]++;
			}
		}
	}
}
...
void printMetrics(int timeSlot, vector< cab > &cabs) {
	...
	printf("Total male passengers: %d\n", totalMalePassengers);
	printf("Total female passengers: %d\n", totalFemalePassengers);
	printf("Rejected male passengers: %d\n", rejectedMalePassengers);
	printf("Rejected female passengers: %d\n", rejectedFemalePassengers);
	printf("Male waiting times:\n");
	for (int i = 0; i <= maxWaitedSlot; i++) {
		printf("Waited %d = %d\n", i, maleWaitingTimes[i]);
	}
	printf("Female waiting times:\n");
	for (int i = 0; i <= maxWaitedSlot; i++) {
		printf("Waited %d = %d\n", i, femaleWaitingTimes[i]);
	}
}
