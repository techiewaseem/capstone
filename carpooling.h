...
pair<vector< event >, double> midPointCalculation(int cabNode, vector< passenger > &passengerOnCab, passenger newPassenger) {
	...
	char cabGender = passengerOnCab[0].gender;  // Get gender of first passenger
	
	// If any passenger has different gender or new passenger doesn't match, return empty path
	for (const passenger& p : passengerOnCab) {
		if (p.gender != cabGender || newPassenger.gender != cabGender) {
			return make_pair(vector<event>(), MAX_DISTANCE);
		}
	}
...
// Helper function to validate gender input
	...
	for (int x = 0; x < costMatrix.size(); x++) {
		...
		for( int i = 0; i < pickups.size(); i++) {
			...
			if( !passengerOnBoard[ x ].size() ) {
				...
				// Check gender compatibility
				char cabGender = passengerOnBoard[x][0].gender;
				
				// Skip if passenger gender doesn't match cab's gender
				bool genderMismatch = false;
				for (const passenger& p : passengerOnBoard[x]) {
					if (p.gender != cabGender || pickups[i].gender != cabGender) {
						genderMismatch = true;
						break;
					}
				}
